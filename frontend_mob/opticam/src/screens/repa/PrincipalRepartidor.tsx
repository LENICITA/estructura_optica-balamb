// src/screens/repartidor/PrincipalRepartidor.tsx
...
export const PrincipalRepartidor = ({ navigation }: Props) => {
  ...
  const cargarPedidos = async () => {
    try {
      ...
      try {
        const response = await apiClient.get('/distribucion/repartidor/pedidos');
        let pedidosData = response.data?.data || response.data?.pedidos || [];

        if (Array.isArray(pedidosData) && pedidosData.length > 0) {
          const pedidosMapeados = pedidosData.map((p: any) => ({
            id: p.id_pedido || p.id,
            cliente: p.cliente_nombre || p.usuario?.nombre_completo || 'Cliente',
            direccion: p.direccion_entrega || p.direccion || '',
            ciudad: p.ciudad_envio || p.ciudad || '',
            latitud: p.latitud || 4.703215,
            longitud: p.longitud || -74.103664,
            estado: p.estado || 'PENDIENTE',
            fecha: p.fecha_pedido || new Date().toLocaleDateString(),
          }));
              ...
              <View style={styles.pedidoContent}>
                ...
                {pedido.estado === 'PENDIENTE' && (
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      console.log('Iniciar entrega:', pedido.id);
                      // TODO: Llamar a API para cambiar estado
                    }}
                  >
                ...
                {pedido.estado === 'EN_ENTREGA' && (
                  <TouchableOpacity
                    style={styles.btnPedido}
                    onPress={() => {
                      navigation.navigate('DetallePedido', { id: pedido.id });
                    }}
                  >
                    <Ionicons name="eye-outline" size={20} color="#fff" />
                    <Text style={styles.btnPedidoTexto}>Ver pedido</Text>
                  </TouchableOpacity>
                )}
