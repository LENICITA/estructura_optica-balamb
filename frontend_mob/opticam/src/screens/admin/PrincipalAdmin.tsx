// src/screens/admin/PrincipalAdmin.tsx
...
export const PrincipalAdmin = ({ navigation }: Props) => {
  ...
  const cargarDatos = async () => {
    try {
      ...
      try {
        // Productos
        const productosRes = await apiClient.get('/inventario/productos');
        const productosData = productosRes.data?.data || productosRes.data?.productos || [];

        // Pedidos
        const pedidosRes = await apiClient.get('/pedidos');
        const pedidosData = pedidosRes.data?.data || pedidosRes.data?.pedidos || [];

        // Usuarios (clientes)
        const usuariosRes = await apiClient.get('/usuarios/clientes/count');
        const totalUsuarios = usuariosRes.data?.data?.total || 0;

        // Ventas (pedidos completados o pagados)
        const ventas = pedidosData.filter((p: any) =>
          p.estado === 'ENTREGADO' || p.estado === 'PAGADO' || p.estado === 'COMPLETADO'
        );
