...
const PruebaMontura = () => {
  ...
  useEffect(() => {
    // ✅ Cargar producto específico si viene por ID
    if (id) {
      const productoId = parseInt(id);
      const productoEncontrado = gafas.find(g => g.id === productoId);
      if (productoEncontrado) {
        setGafaSeleccionada(productoEncontrado);
      } else {
        setGafaSeleccionada(gafas[0]);
      }
    } else if (gafas.length > 0 && !gafaSeleccionada) {
      setGafaSeleccionada(gafas[0]);
    }
  ...
  const volverAtras = () => {
    // Si hay un ID, volver al producto, si no, al catálogo
    if (id) {
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
          ...
          <div className="p-6">
            ...
            <label className="flex items-center gap-2 mb-2">
              <input type="checkbox" onChange={(e) => setTerminosAceptados({...terminosAceptados, terminos: e.target.checked})} /> 
              Acepto los términos y condiciones
            </label>
            <label className="flex items-center gap-2 mb-4">
              <input type="checkbox" onChange={(e) => setTerminosAceptados({...terminosAceptados, privacidad: e.target.checked})} /> 
    ...
    <div className="max-w-[1200px] mx-auto py-10 px-5">
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-2xl shadow-md p-5">
            ...
            {gafas.map(gafa => (
              ...
              >
                ...
                <div>
                  ...
                  <span className="text-[#B90F0F] text-sm">{gafa.precio}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mb-4">
            <label className="font-semibold">⬆️ Posición vertical</label>
            <input 
              type="range" 
              min="-50" 
              max="50" 
          ...
          <button onClick={capturarFoto} className="w-full bg-[#B90F0F] text-white py-3 rounded-xl font-bold mb-2">
            Capturar foto
          </button>
          
          {/* ✅ Cambiar navegación */}
          <button onClick={volverAtras} className="w-full border border-[#B90F0F] text-[#B90F0F] py-3 rounded-xl font-bold">
            Volver a productos
          </button>
        </div>
      </div>
...
export default PruebaMontura; // ✅ Exportación correcta